...
function Sidebar({ isOpen }) {
  ...
  return (
    <aside className={`hidden md:flex flex-col bg-slate-900 border-r border-slate-800 h-full z-40 transition-all duration-300 overflow-hidden ${isOpen ? 'w-16 min-w-[4rem] opacity-100' : 'w-0 min-w-0 opacity-0 border-r-0'}`}>
      <nav className="flex-1 overflow-y-auto py-4 scrollbar-hide">
        <ul className="space-y-3 px-2 flex flex-col items-center">
          {navItems.map((item) => (
